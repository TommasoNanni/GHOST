#!/bin/bash
# Driver for the EgoHumans oracle ablations: one job per (activity, rung).
# Mirrors egohumans_new_driver.sh — submits, waits, resubmits what is unfinished,
# exits when every scene of every requested activity has a dump for every rung.
#
...
#   nohup bash bash_jobs/eval_ablations_egohumans_driver.sh > logs/abl_egohumans_driver.log 2>&1 < /dev/null &
# One activity:
#   nohup bash bash_jobs/eval_ablations_egohumans_driver.sh 06_badminton > logs/abl_driver_badminton.log 2>&1 < /dev/null &
# Subset of rungs:
#   MODALITIES="3,4" nohup bash bash_jobs/eval_ablations_egohumans_driver.sh ... &
# Stop: pkill -f eval_ablations_egohumans_driver
#
# Env:
...
DUMP_ROOT="${DUMP_ROOT:-eval_ablations_egohumans/dumps}"
SCENE_JOB="bash_jobs/eval_ablations_egohumans.sh"
MAXJOBS="${MAXJOBS:-2}"
POLL=180
...
mkdir -p logs
log() { echo "$(date '+%F %T') $*" | tee -a logs/abl_egohumans_driver.log; }
...
log "activities: ${ACTS[*]}   rungs: ${MODS[*]}   maxjobs=$MAXJOBS"
for ACT in "${ACTS[@]}"; do
