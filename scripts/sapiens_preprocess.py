"""Run Sapiens 2D pose estimation on a ghost-pipeline scene.

Reads bounding boxes from json_data/ per camera, crops each person from the
RICH frames, runs Sapiens, and saves per-camera per-person keypoint files:
    {cam_dir}/sapiens_kps_person_{pid}.npz
...
Usage:
    pixi run python scripts/sapiens_preprocess.py \\
...
def _ensure_mount(sqsh: str | None, rich_root: str) -> None:
    ...
    try:
        ...
        pass
    if sqsh is None:
        print(f"[warn] {rich_root} not accessible and --sqsh not given")
        return
    rp.mkdir(parents=True, exist_ok=True)
    print(f"Mounting {sqsh} → {rich_root} …")
    ret = subprocess.run(["squashfuse", sqsh, rich_root])
    if ret.returncode != 0:
        print("[warn] squashfuse failed — frames may be unavailable")
...
def process_camera(cam_dir: Path, scene_name: str, rich_root: str,
    ...
    if not overwrite and all(
        (cam_dir / f"sapiens_kps_person_{pid}.npz").exists() for pid in pids
    ):
        print(f"  [{cam_name}] all sapiens_kps already exist — skipping (use --overwrite)")
        return
    ...
    # Save per-pid
    for pid in pids:
        out_path = cam_dir / f"sapiens_kps_person_{pid}.npz"
