"""Evaluate ghost pipeline on RICH test set — gender-control triplet (M1, M9, M10).

Runs the production pipeline (pred camera + pred scale + pred pose) under
three body-model gender settings, in order:

  M1:  pred FK gendered, GT FK gendered   (gender oracle — upper bound; cheats
       by injecting known subject gender into the prediction)
  M9:  pred FK neutral,  GT FK neutral    (the honest number — matches how
       production actually runs: it never knows subject gender)
  M10: pred FK neutral,  GT FK gendered   (isolates the cost of NOT knowing
       gender: M10 - M9 ≈ neutral/gendered shape mismatch alone)

Production inference always uses the NEUTRAL SMPL-X body model (gender is
unknown at inference time). RICH ground truth is stored as GENDERED SMPL-X
params. M9 is the number to report — for that alone (faster, no gendered
placer/GT pass), see evaluation/evaluate_rich.py. M1 and M10 here exist to
explain the gap between M9 and a hypothetical gender-aware system.

For oracle ablations (GT-scale / GT-camera / GT-pose), see evaluation/ablations.py.
For the placing-approach comparison (FK-DLT vs Sapiens-DLT joint sources), see
evaluation/placing_approaches.py. For the full M1-M10 modality zoo, see
...
Usage
-----
    python evaluation/gender_ablations.py \\
...
# ---------------------------------------------------------------------------
# Per-scene evaluation (M1 -> M9 -> M10)
# ---------------------------------------------------------------------------

_MODALITY_LABELS = {
    1:  "M1: pred-cam + pred-scale + pred-pose (gendered pred + gendered GT)",
    9:  "M9: pred-cam + pred-scale + pred-pose (neutral pred + neutral GT)",
    10: "M10: pred-cam + pred-scale + pred-pose (neutral pred + gendered GT)",
}
...
def evaluate_scene(
    ...
    """Run full inference + M1/M9/M10 evaluation for one scene. Returns metric dict or None."""
    logger.info(f"\n{'─'*60}")
    ...
    # all three modalities: none of M1/M9/M10 substitute GT scale/camera.
    try:
    ...
    # ── 6. Pre-build GT joints — both gendered (M1, M10) and neutral (M9) ───
    ...
    gt_joints, gt_roots = _build_gt_joints(placer)   # gendered — for M1, M10
    ...
    gt_joints_neutral, gt_roots_neutral = _build_gt_joints(placer_neutral)   # neutral — for M9

    # ── 7. Per-modality evaluation loop: M1 -> M9 -> M10 ─────────────────────
    ...
    for modality in _MODALITIES:
        ...
        # M9: fully neutral (pred FK + GT FK) — the honest production number.
        # M10: neutral pred FK + gendered GT FK — isolates the mismatch cost.
        ...
        try:
            trans_dict, orient_dict = active_placer.estimate_procrustes_dlt_mhr(
                scale=pred_scale_pf,
                all_pids=set(all_pids),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate ghost on RICH test set — gender-control triplet (M1, M9, M10)."
    )
    ...
    print(hdr)
    print("  " + "-" * (26 + (col_w + 2) * (len(_MODALITIES) + 1)))
