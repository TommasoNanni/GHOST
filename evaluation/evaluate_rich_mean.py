"""RICH test-set evaluation with CHORDAL-MEAN fusion — naive multi-view baseline.

Byte-for-byte a copy of evaluation/evaluate_rich.py (the M10 production
protocol: neutral pred vs gendered GT, pred camera + pred scale) with EXACTLY ONE
thing changed: the learned PoseFusionModule is replaced by the unweighted chordal
mean of the per-camera SAM3D pose estimates. Nothing else moves — same body data, same
mean-SAM3D betas, same Procrustes-DLT placement, same MapAnything scale, same
metrics — so the gap to evaluate_rich.py is attributable to the fusion model
alone.

How the mean is taken
---------------------
The CHORDAL (projected arithmetic) mean on SO(3):

    R_bar = argmin_R  sum_k  w_k ||R - R_k||_F^2

whose closed form is the arithmetic mean of the rotation MATRICES projected back
onto SO(3) by SVD:  M = U S V^T  ->  R_bar = U diag(1, 1, det(U V^T)) V^T.
The det term keeps the result a proper rotation (no reflection).

Note this is NOT the same as averaging the 6D vectors and letting the downstream
Gram-Schmidt in `_6d_to_aa` re-orthonormalize. That composition drops the third
row before averaging and uses an asymmetric retraction (it preserves b1 exactly
and pushes all the correction into b2/b3), so it does not minimise the Frobenius
criterion above. Measured on 6 RICH test scenes (64.8k joint rotations, 4 cams):
the two agree to a median of 0.040 deg (p95 0.300, p99 1.02, max 19.6) against a
median inter-camera spread of 7.3 deg -- i.e. numerically near-identical, but
only the SVD form is the chordal mean by definition.

Cameras with no detection for a (frame, person) are excluded by the visibility
mask; if NO camera saw them, the slot falls back to the identity rotation (that
(t, p) is invalid downstream anyway).

Per-joint SAM3D confidence is NOT used as a weight — this is the naive
uniform-mean baseline. build_fusion_tensors still builds the confidence tensor
(kept identical to the source script), it is simply not consumed here.
...
Usage
-----
    python evaluation/evaluate_rich_mean.py \\
...
# NOTE: the per-joint confidence channel that PoseFusionModule consumes plays no
# role here — this baseline is an unweighted mean. The remap below is still
# applied when the tensor is built, so build_fusion_tensors stays identical to
# evaluation/evaluate_rich.py.
...
def mean_fuse(pose_t: torch.Tensor, mask_t: torch.Tensor) -> torch.Tensor:
    """Fuse per-camera poses by the CHORDAL mean over the visible cameras.

    Drop-in replacement for `PoseFusionModule.forward` in this script.

    R_bar = argmin_R sum_k w_k ||R - R_k||_F^2, solved in closed form as the SVD
    projection of the arithmetic mean matrix onto SO(3). Unweighted across
    cameras (w_k is the 0/1 visibility mask only) — per-joint SAM3D confidence is
    deliberately not used, this is the naive baseline.
    ...
    w = mask_t[..., None, None, None]                       # (B,T,K,P,1,1,1)

    num = (R * w).sum(dim=2)                                # (B,T,P,J,3,3)
    den = w.sum(dim=2).clamp_min(1e-8)
    M   = num / den
    ...
    # dropped by the visibility mask before any metric sees them.
    empty = (mask_t.sum(dim=2) == 0)[..., None]             # (B,T,P,1) -> broadcast over J
    if bool(empty.any()):
        eye = torch.eye(3, dtype=M.dtype, device=M.device)
        M = torch.where(empty[..., None, None], eye.expand_as(M), M)

    # Chordal projection: nearest rotation to M in Frobenius norm. float32 3x3
    # batched SVD; the det correction blocks reflections when M is near-degenerate.
    U, _, Vh = torch.linalg.svd(M)
    d = torch.linalg.det(U @ Vh)                            # (B,T,P,J)  ±1
    D = torch.eye(3, dtype=M.dtype, device=M.device).expand(*d.shape, 3, 3).clone()
    D[..., 2, 2] = d
    R_bar = U @ D @ Vh                                      # (B,T,P,J,3,3)

    return torch.cat([R_bar[..., 0, :], R_bar[..., 1, :]], dim=-1)


# ---------------------------------------------------------------------------
# Per-scene evaluation (M10 protocol, CHORDAL-MEAN fusion: pred-cam + pred-scale + mean pose)
# ---------------------------------------------------------------------------
...
def evaluate_scene(
    ...
    # ── 2. Fusion: chordal mean over cameras (no learned model) ───────────────
    try:
    ...
    # jconf_t is built by build_fusion_tensors (kept identical to evaluate_rich.py)
    # but deliberately unused: this baseline is an UNWEIGHTED chordal mean.
    with torch.no_grad():
        fused_pose_t = mean_fuse(pose_t.to(device), mask_t.to(device))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate ghost on RICH test set with CHORDAL-MEAN fusion "
                    "(unweighted chordal mean of per-camera SAM3D poses; M10 protocol otherwise)."
    )
    ...
    logger.info("Fusion: CHORDAL MEAN over cameras, unweighted "
                "(no learned model, no checkpoint).")
    ...
    print(f"\n{'='*65}")
    print(f"AGGREGATE  ({len(all_results)} scenes evaluated)  —  CHORDAL-MEAN FUSION "
