"""Evaluate ghost pipeline on RICH test set — placing-approach comparison (M5, M8).

Compares two alternate joint sources feeding the Procrustes-DLT placement
step, in order:

  M5: FK-DLT      — triangulation driven by SMPL-X FK joints instead of MHR70.
  M8: Sapiens-DLT — triangulation driven by Sapiens 2D keypoints instead of MHR70.

Both otherwise match the production config: pred camera + pred scale + pred
pose, neutral pred FK evaluated against gendered GT FK (same convention as
evaluation/ablations.py). Neither substitutes GT camera/scale/pose — only the
joint source feeding DLT triangulation changes. The production default (MHR70
DLT) is `estimate_procrustes_dlt_mhr`, used by evaluate_rich.py /
gender_ablations.py / ablations.py and by the live inference pipeline
(scripts/inference.py) — this script exists to check whether an alternate
joint source would place bodies more accurately.

For the honest production number, see evaluation/evaluate_rich.py (M10).
For the gender-control triplet, see evaluation/gender_ablations.py (M1/M9/M10).
For the progressive oracle ablations, see evaluation/ablations.py (M2/M3/M4).
For the full M1-M10 modality zoo, see evaluation/evaluate_on_rich_test.py.
...
_PLACER_METHODS = {
    5: "estimate_procrustes_dlt_fk",
    8: "estimate_procrustes_dlt_sapiens",
}
...
def evaluate_scene(
    ...
    """Run full inference + M5/M8 evaluation for one scene. Returns metric dict or None."""
    logger.info(f"\n{'─'*60}")
    ...
    # both modalities: neither M5 nor M8 substitute GT scale/camera/pose.
    try:
    ...
    # ── 6. GT joints — gendered model (M5/M8 both use gendered GT, like M2-M4) ──
    ...
    # ── 7. Per-modality evaluation loop: M5 -> M8 ────────────────────────────
    ...
    for modality in _MODALITIES:
        ...
        placer_fn = getattr(placer_neutral, _PLACER_METHODS[modality])
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate ghost on RICH test set — placing-approach comparison (M5, M8)."
    )
