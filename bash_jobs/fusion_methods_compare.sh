...
MODELS="${MODELS:-V2=checkpoints/fusion_module/best.pt R2=checkpoints/fusion_r2/best.pt V3=checkpoints/fusion_v3/best.pt}"
echo "models: $MODELS"
echo "split: ${SPLIT:-test}  max_scenes: ${MAX_SCENES:-52}"
echo ""

pixi run python debug/view_weighting_ceiling_rich.py \
    --split          "${SPLIT:-test}" \
    --max_scenes     "${MAX_SCENES:-52}" \
    --device         cuda \
    --rich_data_root "$MOUNT" \
    --models         $MODELS \
    --out_json       "${OUT:-eval_explainability/fusion_methods_TEST52.json}"
