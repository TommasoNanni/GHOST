"""EgoExo4D progressive oracle ablations — error attribution for the ghost pipeline.
...
Reading the table: M2 - prod = scale error, M3 - M2 = camera error.  The
remaining M3 error is placement-given-perfect-cameras + pose error; the pose
part is read directly off PA-MPJPE, which is placement-invariant.
...
evaluation/evaluate_egoexo.py.

Usage
-----
    pixi run python evaluation/ablations_egoexo.py \\
...
def eval_scene(
    ...
    per_mod = run_fusion_placer(
        ghost_scene_dir, gt_scene_dir, frame_idx, global_groups, fusion_model, device,
