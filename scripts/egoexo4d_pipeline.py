...
All steps are identical to the RICH/EgoHumans pipelines.  Temporal
synchronisation (step 5) gracefully warns when fewer than SYNC_MIN_OVERLAP
frames are available (as expected for single-frame takes).
...
def process_scene(
        ...
        if len(cam_data) < 2:
            ...
            if not pids:
                ...
                for trial in range(SYNC_N_TRIALS):
                    ...
                    print(f"  Trial {trial + 1}/{SYNC_N_TRIALS}  true_shifts={true_shifts}  end_cuts={end_cuts}")
    ...
    if not _reid_already_done:
        ...
        print(f"\n--- Step 8: Skipping re-ID visualisation (cross-view ReID was already done) ---")
