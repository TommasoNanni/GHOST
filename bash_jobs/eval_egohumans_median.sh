#!/bin/bash -l
#SBATCH --job-name=eval_egohumans_median
...
# GEODESIC-MEDIAN fusion (evaluation/evaluate_egohumans_median.py) — no checkpoint.
# The median is independent across frames, so there is no --temporal flag and no
# --joint_conf flag (per-joint confidence is worth 0.0 mm and is not an IRLS
# weight); a flag that silently did nothing would be worse than none.
# Env:
...
SCALE="${SCALE:-pred}"
DUMP_DIR="${DUMP_DIR:-eval_egohumans/dumps_median/${ACTIVITY}}"
...
echo "Job $SLURM_JOB_ID  activity=$ACTIVITY  scale=$SCALE  fusion=geodesic-median  start=$(date)"
nvidia-smi -L
...
pixi run python evaluation/evaluate_egohumans_median.py "${ARGS[@]}"

echo "=== Stage B: aggregate ==="
pixi run python evaluation/evaluate_egohumans_median.py --metrics_only --dump_dir "$DUMP_DIR"
echo "Done: $(date)"
