...
GEODESIC-MEDIAN FUSION (no checkpoint).

Copy of evaluation/ablations.py with the learned PoseFusionModule swapped for
the geodesic median of the per-camera SAM3D poses (median_fuse, identical to
evaluation/evaluate_rich_median.py): the L1 estimator on SO(3), solved by
Weiszfeld/IRLS from a chordal-mean seed. This is the shipped fusion rule, so
this ladder is the one that matches the reported production numbers.
...
ground truth, to attribute total error to a specific stage of the pipeline:
  ...
  M4: GT-cam   + GT-scale   + GT-pose     (full oracle — pose/fusion ceiling)

Reading the table: M2-prod(median) gap = scale error. M3-M2 gap = camera error.
M4-M3 gap = pose error (the irreducible floor given perfect placement; the
median has no learned parameters, so M4 measures SAM3D+placement, not a
fusion-model ceiling).

For the honest production number (no oracle substitutions), see
evaluation/evaluate_rich_median.py. For the v2-module ladder, see
...
Usage
-----
    python evaluation/ablations_median.py \\
...
def median_fuse(pose_t: torch.Tensor, mask_t: torch.Tensor,
    ...
    nothing else — no training, no parameters, no checkpoint.

    Selected on the 12-scene train pool (10 val + 2 train), NOT on test: -0.8 mm
    RR-MPJPE against the chordal mean, beating every trained fusion module.
...
def main() -> None:
    ...
    print(hdr)
    print("  " + "-" * (26 + (col_w + 2) * (len(mods_run) + 1)))
