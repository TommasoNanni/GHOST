...
class BetasAggregator(nn.Module):
    ...
    Input
    -----
    shape       : (B, T, K, P, 10) — per-camera per-frame predicted betas
    person_mask : (B, T, K, P)     — 1 where the person was detected
...
class FusionWithBetas(nn.Module):
    """Wraps PoseFusionModule + BetasAggregator into a single trainable unit.

    forward() returns a 2-tuple (pose_aggr, betas_out) so downstream losses
    can address each output by index.

    Parameters
    ----------
    pose_module       : PoseFusionModule instance.
    betas_aggregator  : BetasAggregator instance.
    """

    def __init__(
        self,
        pose_module: PoseFusionModule,
        betas_aggregator: BetasAggregator,
    ):
        super().__init__()
        self.pose_module = pose_module
        self.betas_aggregator = betas_aggregator
    ...
    def forward(
        ...
        pose        : (B, T, K, P, J, 6)
        person_mask : (B, T, K, P)
        joint_mask  : (B, T, K, P, J) or None
        shape       : (B, T, K, P, 10) or None

        Returns
        -------
        pose_aggr   : (B, T, P, J, 6)
        betas_out   : (B, P, 10) or None when shape is not provided
        """
        pose_aggr = self.pose_module(pose, person_mask, joint_mask)
        betas_out = self.betas_aggregator(shape, person_mask) if shape is not None else None
        return pose_aggr, betas_out
