"""RICH test-set evaluation with GEODESIC-MEDIAN fusion — the shipped fusion rule.
...
thing changed: the learned PoseFusionModule is replaced by the GEODESIC MEDIAN of
the per-camera SAM3D pose estimates. Nothing else moves — same body data, same
mean-SAM3D betas, same Procrustes-DLT placement, same MapAnything scale, same
metrics — so the gap to evaluate_rich.py and to evaluate_rich_mean.py is
attributable to the fusion rule alone.

Why the median and not the mean
-------------------------------
The chordal mean minimises an L2 criterion, sum_k ||R - R_k||_F^2, so one badly
...
chordal mean with weights w_k / (theta_k + eps). Same closed-form SVD projection
each step, so the cost is a handful of extra batched 3x3 SVDs.

Measured on the 12-scene selection pool (10 val + 2 train), RR-MPJPE:

    chordal mean (previous baseline)   48.5 mm
    Karcher (intrinsic) mean           48.6 mm   (+0.0 — a non-event)
    GEODESIC MEDIAN                    47.8 mm   (-0.8)
    Tukey biweight                     47.9 mm   (-0.6)
    trim worst view                    48.1 mm   (-0.5)
    oracle per-joint camera choice     42.8 mm   (-5.7)

The whole gain is L2 -> L1; extrinsic vs intrinsic averaging is worth nothing.
Gentle beats hard consistently: the median beats the biweight beats trimming —
with 4-8 views, discarding any costs more variance than it saves in bias.

The estimator was chosen on the TRAIN pool, so a test run is a clean report, not
selection. Do NOT shop across estimators on test.
...
Per-joint SAM3D confidence is NOT used as a weight. It was measured on all 52
test scenes (1,068,444 slots): within-slot Spearman -0.078, top-1 29.2% against
~17-25% chance, and worth -0.0 mm end-to-end at conf^1 while being monotonically
HARMFUL as it is trusted more (+0.4 at conf^2, +1.3 at conf^4).
build_fusion_tensors still builds the confidence tensor (kept identical to the
source script), it is simply not consumed here.
...
def median_fuse(pose_t: torch.Tensor, mask_t: torch.Tensor,
    ...
    Drop-in replacement for `PoseFusionModule.forward` in this script, and for
    `mean_fuse` in evaluation/evaluate_rich_mean.py.
    ...
    Selected on the 12-scene train pool (10 val + 2 train), NOT on test: -0.8 mm
    RR-MPJPE against the chordal mean, beating every trained fusion module. The
    intrinsic (Karcher) MEAN was also measured and is a non-event (+0.0 mm) — the
    entire gain comes from L2 -> L1, not from extrinsic vs intrinsic averaging.
...
# ---------------------------------------------------------------------------
# Per-scene evaluation (M10 protocol, GEODESIC-MEDIAN fusion: pred-cam + pred-scale + median pose)
# ---------------------------------------------------------------------------
...
def evaluate_scene(
    ...
    # ── 2. Fusion: geodesic median over cameras (no learned model) ────────────
    try:
    ...
    # but deliberately unused: per-joint SAM3D confidence was measured to be worth
    # 0.0 mm end-to-end and monotonically harmful when sharpened, so the IRLS
    # weights here are the 0/1 visibility mask only.
    with torch.no_grad():
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate ghost on RICH test set with GEODESIC-MEDIAN fusion "
                    "(L1 Weiszfeld median of per-camera SAM3D poses; M10 protocol otherwise)."
    )
    ...
    print(f"\n{'='*65}")
    print(f"AGGREGATE  ({len(all_results)} scenes evaluated)  —  GEODESIC-MEDIAN FUSION "
