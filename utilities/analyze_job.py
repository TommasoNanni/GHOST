...
def analyze_predictions(d: dict[str, np.ndarray]) -> None:
    ...
    # ── CAMERA ───────────────────────────────────────────────────────────────
    print("\n── CAMERA (quat+trans+focal) ────────────────────────────")
    cam_mse = ((camera - gt_camera) ** 2).mean()

    # Camera collapse: how similar are predictions across K cameras?
    # If collapsed, std across K at each frame should be near zero.
    cam_cross_k_std = camera.std(axis=1).mean()           # std over K per frame
    gt_cross_k_std  = gt_camera.std(axis=1).mean()
    cam_temporal_std = camera.std(axis=0).mean()          # std over T per camera
    gt_temporal_std  = gt_camera.std(axis=0).mean()

    print(f"  MSE(pred, GT)        :  {cam_mse:.6f}")
    print(f"  Pred std across K    :  {cam_cross_k_std:.6f}   (GT: {gt_cross_k_std:.6f})")
    print(f"  Pred std across T    :  {cam_temporal_std:.6f}   (GT: {gt_temporal_std:.6f})")

    # Per-camera MSE
    for k in range(K):
        cam_k_mse = ((camera[:, k] - gt_camera[:, k]) ** 2).mean()
        print(f"  Camera {k} MSE: {cam_k_mse:.6f}")

    # Translation error (metres): camera centre = -R^T @ t
    # quat → matrix via Rodrigues (scipy not available here — use numpy trick)
    try:
        ...
        print(f"\n  Translation error (no alignment):")
        for k in range(K):
