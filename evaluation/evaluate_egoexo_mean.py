"""Evaluate ghost on EgoExo4D val with CHORDAL-MEAN fusion — naive multi-view baseline.

Copy of evaluation/evaluate_egoexo.py with EXACTLY ONE thing changed: the learned
PoseFusionModule is replaced by the unweighted chordal mean of the per-camera
SAM3D poses. Same GT, same auto-matched subject, same Procrustes-DLT placement,
same scale, same metrics — so the gap to evaluate_egoexo.py is attributable to
the fusion model alone. The RICH counterpart is evaluation/evaluate_rich_mean.py.

The chordal (projected arithmetic) mean on SO(3):

    R_bar = argmin_R  sum_k  w_k ||R - R_k||_F^2

solved in closed form as the arithmetic mean of the rotation MATRICES projected
back onto SO(3) by SVD:  M = U S V^T -> R_bar = U diag(1, 1, det(U V^T)) V^T.
Weights w_k are the 0/1 visibility mask only — per-joint SAM3D confidence is
deliberately unused, this is the naive baseline. Identical estimator to
`chordal_mean` in evaluation/explainability_q1.py (verified: agreement 5e-05 deg
in float32, 5e-08 deg in float64).
...
def mean_fuse(pose_t: torch.Tensor, mask_t: torch.Tensor) -> torch.Tensor:
    ...
    to `mean_fuse` in evaluation/evaluate_rich_mean.py.
    ...
    cameras (w_k is the 0/1 visibility mask only).
    ...
    # identity so the decomposition stays well-conditioned; these slots carry no
    # placed prediction downstream.
    ...
    # Chordal projection: nearest rotation to M in Frobenius norm. The det term
    # blocks reflections when M is near-degenerate.
...
def run_fusion_placer(
    ...
    with torch.no_grad():
        fused_t = mean_fuse(pose_t.to(device), mask_t.to(device))
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate ghost on EgoExo4D val with CHORDAL-MEAN fusion "
                    "(unweighted chordal mean of per-camera SAM3D poses; no checkpoint)")
    ...
    print("\n" + "=" * 60)
    print(f"EgoExo4D evaluation — CHORDAL-MEAN FUSION — "
          f"{len(results)} body-GT scenes  [scale={args.scale}]")
