#!/usr/bin/env python
"""EgoHumans progressive oracle ablations — error attribution for the ghost pipeline.
...
For the honest production number see evaluation/evaluate_egohumans.py.
...
-----
  ...
  pixi run python evaluation/ablations_egohumans.py \
      ...
      --scene 031_badminton --dump_dir eval_ablations_egohumans/dumps

  # full run (resumable), then aggregate
  pixi run python evaluation/ablations_egohumans.py --ghost_root ... --gt_root ... \
      --checkpoint ... --dump_dir eval_ablations_egohumans/dumps
  pixi run python evaluation/ablations_egohumans.py --metrics_only \
      --dump_dir eval_ablations_egohumans/dumps
"""
...
def predict_scene(ghost_scene: Path, frames, pids, fusion_model, device, smplx_arg,
                  cam_pos_aria, cam_w2c, gt_pose_by_frame, modalities, temporal=False):
...
def eval_scene(ghost_scene: Path, gt_scene: Path, fusion_model, device, smplx_arg,
               modalities=None, temporal=False):
    ...
    per_mod, raw_coco, cam_names = predict_scene(
        ghost_scene, frames, pids, fusion_model, device, smplx_arg,
        cam_pos_aria, cam_w2c, gt_pose_by_frame, modalities, temporal)
    if not per_mod:
...
def main():
    ...
    ap.add_argument("--dump_dir", default="eval_ablations_egohumans/dumps")
    ...
    for scene in scenes:
        ...
        try:
            d = eval_scene(ghost_root / scene, gt_root / scene, fusion_model, device,
                           smplx_arg, args.modalities, args.temporal)
        except Exception as e:
